...
- ✅ Step 4 (subscriptions + trial gating) DONE 2026-07-06: global `plans` table (NOT tenant-scoped) seeded Basic(25000 IQD/mo) + Pro(50000). Added `tenants.subscription_ends_at`. **`Tenant::hasActiveAccess()`** = (status=trial && trial_ends_at future) OR (status=active && subscription_ends_at future); else false (suspended/cancelled). **`EnsureSubscribed` middleware** (alias `subscribed`, registered in bootstrap/app.php) → returns **402** when lapsed; wraps ONLY the business routes (catalog/staff/orders/sync). Auth + subscription routes stay OUTSIDE the gate so a lapsed owner can still log in & pay. Endpoints: `GET /plans` (public), `GET /subscription` (status+days_remaining), `POST /subscribe` (**admin only**, 403 else; sim payment → status=active, +30 days; early renew STACKS onto remaining time). Verified via curl: trial→200, expire→402, /me still 200 when lapsed, subscribe→200 again, bad plan 422, manager subscribe 403.
- ⚠️ `POST /sync` is currently BEHIND the subscription gate → a lapsed tenant can't push offline sales. Conflicts with offline-first priority. FUTURE: let sync PUSH through during a grace period so no sale is ever lost.
- ⚠️ PINs stored **PLAINTEXT** (seeder + `unique(tenant_id,pin)` DB constraint depend on it; pin-login does direct `where('pin',...)` lookup). Decision: keep for now (4-digit = low entropy anyway). Harden later = decide hash vs uniqueness tradeoff before production.
- ⏭️ Next: **ONE Vue web app** at `/workspaces/easycasher-saas/dashboard` with TWO areas — owner/manager dashboard + web POS (PWA, offline-first via IndexedDB → `/api/sync`). NO Flutter (see [[feedback]] 2026-07-06). Then Talabat/Careem.

## API surface so far (public: register, login, pin-login, plans; rest auth:sanctum)
POST /register, POST /login, POST /pin-login, GET /plans
POST /logout, GET /me, GET /subscription, POST /subscribe  (auth, NOT subscription-gated)
...
- ✅ Step 5 (restaurant settings) DONE 2026-07-06: `tenants.settings` JSONB column + `Tenant::DEFAULT_SETTINGS` (currency IQD, tax_rate 0, tax_inclusive false, service_mode quick_service, address, phone) merged via `getSettingsAttribute` accessor (kept alongside `'settings'=>'array'` cast — accessor wins on get, cast json-encodes on set). Endpoints (auth, NOT subscription-gated, next to /subscription): `GET /settings`, `PUT /settings` (admin/manager only → 403 else; partial-merge). Settings also ride along in `/me` tenant JSON (so POS caches tax offline). Demo seed tenant now has tax_rate 15, full_service, Erbil address. Verified: defaults, partial update, /me reflects, cashier 403.
- 🐛 FIXED 2026-07-07 (same class, orders): an open dine-in tab sends `method:""` → ConvertEmptyStringsToNull → null → NOT NULL violation → open-tab push failed. Fix in `SyncController::pushOrders`: `$orderFields['method'] ??= 'cash'`. (Watch for this pattern on ANY NOT-NULL string column that can arrive empty.)
- 🔧 2026-07-07: `ReportController` base query changed `status != 'void'` → `status = 'completed'` so open dine-in tabs (active) don't inflate revenue.
- 🐛 FIXED 2026-07-06 (pre-existing sync bug): items with NO modifiers send `modifiers_label:""` → Laravel `ConvertEmptyStringsToNull` middleware turns it to null → violated NOT NULL column → 500 on MOST real orders. F3 test missed it (only tested items WITH modifiers). Fix in `SyncController::pushOrders`: `$fields['modifiers_label'] ??= ''`. Verified: split order w/ empty-modifier item + tax/discount/tip now lands fine.
- **Now on GitHub (2026-07-06):** `api/` + `dashboard/` live in ONE private monorepo **github.com/shahohabib87/easycasher-saas** (default branch `main`). `.env`/`vendor`/`node_modules` all gitignored (verified no secrets committed). Still ask before pushing NEW code (per [[feedback]]).
- **⚠️ PUSH GOTCHA (Codespace) — the fix:** Claude's Bash shell + the Codespace default `GITHUB_TOKEN` are scoped to the Saas-POS repo only → **cannot push to easycasher-saas (403)**. Claude does `git commit` locally; the **USER must push from THEIR VS Code terminal** with this one-liner: `unset GITHUB_TOKEN && gh auth setup-git && git push` (the unset is essential — both git & gh grab GITHUB_TOKEN first otherwise). One-time `gh auth login` (browser) already done. Verify from Claude side: `git rev-parse main` == `git rev-parse origin/main` (push updates the local tracking ref even though Claude can't push).
