...
## Step F17 — DELIVERY DISPATCH (2026-07-09, commit `55efaf6`) — Flutter parity add-on
- User showed Flutter Delivery screen (3 panels: Drivers | Cash Float | Delivery Areas) + Add Driver (name*/phone) & Add Delivery Area (name*/fee 0=free) dialogs. Built to match.
- Backend: `drivers`(name/phone/active) + `delivery_areas`(name/fee) tables/models/CRUD (tenant-scoped, softdeletes); `orders.driver_id`; `GET /deliveries` (active delivery orders w/ driver + items) + `POST /orders/{order}/driver` (assign/clear). (Order customer_name/phone/delivery_address/delivery_notes fields were already added by USER earlier — migration 2026_07_09_120000.)
- Frontend: `api/delivery.ts` (driverApi/deliveryAreaApi/deliveriesApi); types Driver, DeliveryArea + Order gained driver_id/driver. **DeliveryView.vue rebuilt** as 3-panel dispatch: Drivers (list+Add+delete, empty "No drivers yet"), Cash Float ("N drivers out · IQD to collect", active deliveries w/ customer+address + assign-driver dropdown, empty "No active deliveries"), Delivery Areas (list+Add+delete, Free if fee 0, empty "No areas yet"). Manager-gated add/delete. (Replaces the old Own/Talabat tabs layout.)
- ✅ Verified curl: driver create returns active; delivery order shows in /deliveries w/ customer info; assign driver sets order.driver.

## SESSION 2026-07-14 (afternoon) — delivery form v3 + online-orders panel fix
- **POS delivery form v3** (user: "redesign still not good" → picked "form still cramped"): one bordered emerald card in PosView.vue — full-width phone (rose border while empty) + full-width name, driver|area selects one row, rounded-lg 13px inputs (dropped rounded-full pills). Collapse-to-summary behavior kept (auto-open on Delivery/missing phone, auto-collapse on item add).
- **Online Orders detail box tiny (user screenshot):** `h-[calc(100vh-7rem)]` locked page to viewport → on short screens detail panel's flex-1 scroll area crushed to ~90px (only Status/Placed visible). Fix: added `min-h-[44rem]` floor — AppLayout main is overflow-auto so page scrolls instead. Only view with this pattern.
- **⚠️ PENDING PUSH: easycasher-saas main ahead 6** (menu profit report w/ ReportController+route, recipe-picker fixes, delivery form v2+v3, online-orders fix; dist/ built+committed). Codespace token 403s — user pushes, then droplet: `git fetch origin main && git reset --hard origin/main` + `\cp -rf dashboard/dist/. /var/www/html/` + `systemctl restart easycasher-api` (no migrations).
