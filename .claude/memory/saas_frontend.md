...
## ⏳ STILL REMAINING for literal 100% Flutter parity (ONE big piece):
- **KOT model** — Flutter tracks each table as a stack of Kitchen Order Tickets (multiple send-to-kitchen rounds per table, each with own status). Unlocks: incremental KOTs / "sent" history, per-item void of an already-sent ticket, table-transfer dialog, live kitchen-status chip in the cart. Web currently uses a simpler single-open-tab-per-table model (reopen-to-edit covers most of it). Building this = new `kots` table (order→many KOTs→items) + KDS/cart/orders rewired. NOT yet built — biggest remaining chunk; flagged to user 2026-07-07 for go/no-go.
