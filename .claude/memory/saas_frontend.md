...
## Next
- ⏭️ Step F2: dashboard shell (persistent layout + nav sidebar, subscription banner using `GET /subscription`, then Staff + Menu screens hitting the existing CRUD APIs).
- Then Step F3: POS register (menu grid → cart → payment → `/sync`). Then F4: offline PWA.
